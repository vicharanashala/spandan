...
router.post('/', authorize('student'), async (req, res) => {
  try {
    ...
    res.status(201).json({
      success: true,
      response: question.silentMode && !question.resultsRevealed
        ? { ...response.toObject(), isCorrect: undefined, points: undefined }
        : { ...response.toObject(), isCorrect, points }
    })
  } catch (error) {
...
// Teacher-only participation heatmap: every member x question, with response and confidence state.
router.get('/room/:roomId/participation', authorize('teacher'), async (req, res) => {
  try {
    ...
    if (!room || room.teacher.toString() !== req.user._id.toString()) return res.status(403).json({ error: 'Only the room owner can view participation' })
    const [members, questions, responses] = await Promise.all([
      RoomMember.find({ roomId: room._id }).populate('studentId', 'name email').lean(),
      Question.find({ roomId: room._id }).sort({ createdAt: 1 }).lean(),
      Response.find({ roomId: room._id }).lean()
    ])
    const cells = Object.fromEntries(responses.map(r => [`${r.studentId}_${r.questionId}`, { answered: true, isCorrect: r.isCorrect, confidence: r.confidence }]))
    res.json({ success: true, students: members.map(m => ({ _id: m.studentId._id, name: m.studentId.name, email: m.studentId.email })), questions, cells })
  } catch (error) {
    res.status(500).json({ success: false, error: 'Failed to load participation analytics' })
  }
})
    ...
    const questionsWithResponses = questions.map(q => {
      ...
      return {
        ...
        ...(studentResponse && {
          ...
          isCorrect: (!q.silentMode || q.resultsRevealed) ? studentResponse.isCorrect : undefined,
          responseTime: studentResponse.responseTime,
          pointsEarned: (!q.silentMode || q.resultsRevealed) ? studentResponse.points : undefined
        }),
...
// GET /api/responses/room/:roomId/student-detailed - Per-student grid data for export
// Teacher only (must own the room). Returns all students in the room + all their responses.
router.get('/room/:roomId/student-detailed', async (req, res) => {
  try {
    const Response = (await import('../models/Response.js')).default
    const Question = (await import('../models/Question.js')).default
    ...
    // All students who joined this room OR answered at least one question
    const memberRecords = await RoomMember.find({ roomId: toObjectId(roomId) })
    const responderIds = await Response.distinct('studentId', { roomId: toObjectId(roomId) })
    const allStudentIds = [...new Set([
      ...memberRecords.map(m => m.studentId.toString()),
      ...responderIds.map(id => id.toString())
    ...
    const responses = await Response.find({ roomId: toObjectId(roomId) }).lean()

    res.json({ success: true, students, responses })
  } catch (error) {
    console.error('Error fetching student-detailed export data:', error)
