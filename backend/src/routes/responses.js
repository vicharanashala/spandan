...
router.get('/stats/student/:studentId', async (req, res) => {
  try {
    ...
    // Count launched polls: questions with 'approved' status AND actually launched to students
    // (launchedAt set) — an approved-but-not-yet-started question in the bank shouldn't count
    // as a "missed" poll for students who simply haven't seen it yet.
...
router.get('/room/:roomId/student/:studentId', async (req, res) => {
  try {
    ...
    // Get all approved questions that have actually been LAUNCHED to students (launchedAt set).
    // An approved-but-not-yet-started question sits in the teacher's question bank and should
    // NOT appear in a student's past-questions list until it's actually been asked.
