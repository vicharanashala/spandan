...
async function generateWithMiniMax(prompt) {
  ...
  if (!response.ok) {
    ...
    console.error('[gen:minimax] API ERROR:', errorData)

    throw new Error(
      `MiniMax API error: ${response.status} - ${errorData}`
    )
  }
  ...
  console.log(
    `[gen:minimax] content length=${content.length}`
  )

  console.log(
    `[gen:minimax] reasoning length=${reasoning.length}`
  )

  /*
   * M2.7 may put the actual answer in content,
   * while reasoning_content contains the internal reasoning.
   *
   * We should prefer content.
   */
  if (content.trim()) {
    return content.trim()
  }
  ...
  /*
   * Fallback only if content is empty.
   */
  if (reasoning.trim()) {
    console.warn(
      '[gen:minimax] content is empty, using reasoning_content'
    )
  ...
  throw new Error(
    ...
    JSON.stringify(choice).slice(0, 2000)
  )
}
