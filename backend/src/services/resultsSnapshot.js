...
export async function buildSnapshot(roomId) {
  ...
  // Approved + actually-launched questions, newest-first — the exact set + order the
  // per-student endpoint renders. launchedAt excludes questions still sitting in the bank that
  // were approved but never started, so they don't leak into a student's past-questions list.
  const approved = allQuestions
    .filter((q) => q.status === 'approved' && q.launchedAt)
