...
import { flushAndCleanRoom } from './chatService.js'
...
export const updateRoom = async (roomId, updates) => {
  ...
  if (updates && (updates.isActive === false || updates.endedAt)) {
    ...
    await flushAndCleanRoom(roomId)
  }
...
export const deleteRoom = async (roomId) => {
  ...
  // Step 1: Flush any pending buffered chat messages to MongoDB and delete Redis keys FIRST.
  await flushAndCleanRoom(roomId)

  // Step 2: Cascade-delete everything tied to this room so no orphaned records are left behind.
  ...
  const [responses, members, questions, transcripts, chatMessages] = await Promise.all([
  ...
  console.log(
    ...
    `${chatMessages.deletedCount} chatMessages`
  )
  return room
}
