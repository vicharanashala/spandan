...
export const getRoomById = async (id) => {
  const room = await Room.findById(id).populate('teacher', 'name email').populate('currentQuestion')
  if (!room) {
...
export const getRoomByCode = async (code) => {
  const room = await Room.findOne({ code: code.toUpperCase() }).populate('teacher', 'name email').populate('currentQuestion')
  if (!room) {
    throw new Error('Room not found')
  }
  return room
