...
export const useSocketStore = create((set, get) => ({
  ...
  connect: (token) => {
    ...
    const socket = io(SOCKET_URL, {
      auth: { token },
      // The backend's Socket.IO server (backend/src/index.js) never sets a custom `path`, so it
      // listens on the library default '/socket.io'. This used to be hardcoded to
      // '/spandan/socket.io', which the backend was never actually serving on and which the Vite
      // dev proxy (below, plain '/socket.io') never matched either — so locally the socket never
      // connected at all and the UI sat stuck on "Reconnecting...". Using the real default path
      // lets the existing Vite proxy rule reach the backend correctly.
